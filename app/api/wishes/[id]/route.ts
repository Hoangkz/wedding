...
export async function PUT(req: NextRequest, context: Context) {
    ...
    try {
        ...
        console.error(`Lỗi khi cập nhật lời chúc ID: ${id}`);
        return NextResponse.json({
...
export async function DELETE(req: NextRequest, { params }: Context) {
    ...
    try {
        ...
        return NextResponse.json({ message: `Lời chúc ID ${id} đã được xóa thành công.` }, { status: 200 });

    } catch {
        console.error(`Lỗi khi xóa lời chúc ID: ${id}`);
        return NextResponse.json({
