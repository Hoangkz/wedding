...
export default function CustomerManagementPage() {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        const payload = {
            ...editingCustomer,
            attended: typeof editingCustomer.attended === 'string' ?
    ...
    const handleDelete = async (customerId: string, customerName: string) => {
        if (window.confirm(`Bạn có chắc chắn muốn xóa khách hàng "${customerName}"? Thao tác này không thể hoàn tác!`)) {
            try {
                            ...
                            {paginatedCustomers.map((customer) => (
                                <tr key={customer.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="p-3 text-sm">
                                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${customer.type === 'Groom' ? 'bg-blue-100 text-blue-800' :
                                            customer.type === 'Bride' ? 'bg-pink-100 text-pink-800' :
                                                'bg-gray-100 text-gray-800'
                                            }`}>
                                            {CUSTOMER_TYPES.find(t => t.value === customer.type)?.label || 'N/A'}
                                        </span>
                                    </td>
                                    <td className="p-3 text-sm text-gray-700 truncate max-w-xs">{customer.invitation}</td>
                                    <td className="p-3 text-sm text-gray-700">{formatDateTime(customer.invitedAt)}</td>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/60 z-50 p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
                ...
                <div className="flex-grow overflow-y-auto p-6">
                    <form onSubmit={onSave} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <Input label="Tên Khách Hàng *" name="name" value={customer.name} onChange={onChange} />
                ...
                <div className="flex justify-end space-x-3 border-t pt-4 p-6 sticky bottom-0 bg-white z-10">
                    ...
                    <Button type="submit" onClick={onSave} className="cursor-pointer bg-blue-600 text-white hover:bg-blue-700">
